...
public class CountryController {
	...
	public ModelAndView getList(Country country,
			...
			HttpSession session) {
		session.setAttribute("111", 22);
		ModelAndView result = new ModelAndView(page_list);
		List<Country> countryList = countryService.selectByCountry(country,
				page, rows);
	...
	public ModelAndView pageInfo(CountryQueryModel queryModel) {
		...
		PageInfo<Country> pageInfo = countryService
				.selectByCountryQueryModel(queryModel);
